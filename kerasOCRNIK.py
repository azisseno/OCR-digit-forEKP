...
def crop_image_random(image):
    ...
    intY = int(y)
    intX = int(x)
    return image[intY:intY + img_size, intX:intX + img_size]
...
def prepare_datasets(samples):
    ...
    round_int = int(round(len(samples) / 10 * 9))
    training_set = samples[:round_int]
    test_set = samples[round_int:]
...
if __name__ == '__main__':
    ...
    classifier.save('model/keras_alphanumeric.h5')
    ...
    coreml_mnist = coremltools.converters.keras.convert(
        'model/keras_alphanumeric.h5',
        input_names=['image'],
        output_names=['output'],
        class_labels=output_labels,
        image_input_names='image')
    coreml_mnist.author = 'azissenoaji'
    coreml_mnist.license = 'Venazp'
    coreml_mnist.short_description = 'OCR for NIK EKTP'
    coreml_mnist.input_description['image'] = 'NIK eKTP'
    coreml_mnist.output_description['output'] = 'probability'
    coreml_mnist.output_description['classLabel'] = 'string'

    coreml_mnist.save('model/OCRForNikETKP.mlmodel')

    converter = lite.TFLiteConverter.from_keras_model_file('model/keras_alphanumeric.h5')
    ...
    id = uuid.uuid4().hex
    file = open(directory + 'model_' + id + '.tflite', 'wb')
    file.write(model)
